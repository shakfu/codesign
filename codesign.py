...
def sign(config):
    ...
    args = {
        ...
        "entitlements": f"--entitlements {config['package_details']['entitlements']}"
        if config["package_details"]["entitlements"]
        else None,
...
def package(config, package_debug=False):
    ...
    if not package_debug:
        ...
        print("Package debugging active:")
    ...
    while not notarized:
        ...
        if stdout:
            ...
            try:
                ...
                logging.debug(
                    f"inconclusive notarization status data returned: {status}"
                )
...
def main():
    ...
    if args.notarize_only or run_all and not halt:
        ...
        if r == 0:
            ...
            print(
                f"xcrun notarytool history --keychain-profile {config['identification']['keychain-profile']}"
            )
            halt = True
